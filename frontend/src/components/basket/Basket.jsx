...
const Basket = () => {
  ...
  useEffect(() => {
    if (didMount && basket.length !== 0) {
      // kernel.saveBasketItems(basket, kernel.auth.currentUser.uid)
      //   .then(() => {
        ...
        <div className="basket-checkout">
          <div className="basket-total">
            ...
            <h2 className="basket-total-amount">
              {displayMoney(calculateTotal(basket.map((product) => parseFloat(product.price) * parseFloat(product.quantity))))}
            </h2>
          </div>
          <button
