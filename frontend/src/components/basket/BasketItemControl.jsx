...
const BasketItemControl = ({ product }) => {
  ...
  const onAddQty = () => {
    ...
    if (parseFloat(product.quantity) < parseFloat(product.maxQuantity)) {
      dispatch(addQtyItem(product.id));
    }
  };

  const onMinusQty = () => {
    if ((parseFloat(product.maxQuantity) >= parseFloat(product.quantity)) && parseFloat(product.quantity) !== 0) {
  ...
  return (
    <div className="basket-item-control">
      <button
        ...
        disabled={parseFloat(product.maxQuantity) === parseFloat(product.quantity)}
        onClick={onAddQty}
        type="button"
      >
      ...
      <button
        ...
        disabled={parseFloat(product.quantity) === 1}
        onClick={onMinusQty}
        type="button"
      >
