  ...
  switch (type) {
    case UPDATE_EMAIL: {
      try {
        ...
        yield call(kernel.updateEmail, payload.password, payload.newEmail);
    ...
    case UPDATE_PROFILE: {
      try {
        ...
        if (email && password && email !== state.profile.email) {
          yield call(kernel.updateEmail, password, email);
        }

        if (avatarFile || bannerFile) {
          const bannerURL = bannerFile ? yield call(kernel.storeImage, state.auth.id, 'banner', bannerFile) : payload.updates.banner;
          const avatarURL = avatarFile ? yield call(kernel.storeImage, state.auth.id, 'avatar', avatarFile) : payload.updates.avatar;
          const updates = { ...payload.updates, avatar: avatarURL, banner: bannerURL };

          yield call(kernel.updateProfile, state.auth.id, updates);
          yield put(updateProfileSuccess(updates));
        } else {
          yield call(kernel.updateProfile, state.auth.id, payload.updates);
